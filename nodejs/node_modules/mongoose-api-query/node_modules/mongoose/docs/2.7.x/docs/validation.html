  ...
  <body>
    ...
    <div id="wrap">
      <div id="page">
        ...
        <div id="content">
          <!-- ANNOUNCE -->
          <h1>Validation in models</h1>

<p>Before we get into the specifics of validation syntax, please keep the following rules in mind:</p>

<ul>
<li><p>Validation is defined in the <code>Schema</code></p></li>
<li><p>Validation occurs when a document attempts to be saved, after defaults have been applied.</p></li>
<li><p>Mongoose doesn't care about complex error message construction. Errors have type identifiers. For example, <code>"min"</code> is the identifier for the error triggered when a number doesn't meet the minimum value. The path and value that triggered the error can be accessed in the <code>ValidationError</code> object.</p></li>
<li><p>Validation is an internal piece of middleware</p></li>
<li><p>Validation is asynchronously recursive: when you call <code>Model#save</code>, embedded documents validation is executed. If an error happens, your <code>Model#save</code> callback receives it.</p></li>
</ul>

<h2>Simple validation</h2>

<p>Simple validation is declared by passing a function to <code>validate</code> and an error type to your <code>SchemaType</code> (please read the chapter on <a href="model-definition.html">model definition</a> to learn more about schemas).</p>
...
<p>If you find this syntax too clumsy, you can also define the type</p>
...
<p>If you want to test a certain value against a regular expression:</p>
...
<p>If you define a validator function with two parameters, like:</p>
...
<p>Then the function <code>fn</code> has to be called with <code>true</code> or <code>false</code>, depending on whether the validator passed. This allows for calling other models and querying data asynchronously from your validator.</p>
...
<ul>
<li><p><code>enum</code>: takes a list of allowed values. Example:</p>

<pre><code>var Post = new Schema({
...
<ul>
<li><p><code>min</code>: minimum value</p>

<pre><code>var Person = new Schema({
    age: { type: Number, min: 5 }
})
</code></pre></li>
<li><p><code>max</code>: maxmimum value</p>
