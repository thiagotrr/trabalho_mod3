...
<p>The schema constructor takes a second options argument. There are currently two available options:</p>
...
<p>This option is passed to mongodb with all operations and let's us specify if errors should be returned to our callbacks as well as tune write behavior.</p>

<pre><code>var safe = true;
new Schema({ .. }, { safe: safe })
</code></pre>

<p>By default this is set to <code>true</code> for all schemas which guarentees that any error that occurs will get reported back to our method callback.</p>

<p>By setting <code>safe</code> to something else like <code>{ j: 1, w: 2, wtimeout: 10000 }</code> we can guarantee the write was committed to the journal <em>(j: 1)</em>, at least 2 replicas <em>(w: 2)</em>, and that the write will timeout if it takes longer than 10 seconds <em>(wtimeout: 10000)</em>. Errors will still be reported back to our callback.</p>

<p>There are other options like <code>{ w: "majority" }</code> too. See the <a href="http://www.mongodb.org/display/DOCS/Verifying+Propagation+of+Writes+with+getLastError">MongoDB docs</a> for more detail.</p>

<pre><code>var safe = { w: "majority", wtimeout: 10000 };
new Schema({ .. }, { safe: safe })
</code></pre>

<h3>strict</h3>

<p>The strict option makes it possible to ensure that values added to our model instance that were not specified in our schema do not get saved to the db.</p>

<pre><code>var thing = new Thing({ iAmNotInTheSchema: true });
thing.save() // iAmNotInTheSchema was saved to the db @#$%^!!
</code></pre>

<p>By default this is <code>false</code> for backward compatibility. Set it to true and sleep peacefully.</p>

<pre><code>new Schema({ .. }, { strict: true })
</code></pre>

<p>This value can also be overridden at the model instance level by passing a second boolean argument:</p>

<pre><code>var Thing = db.model('Thing');
var thing = new Thing(doc, true);  // enables strict mode
var thing = new Thing(doc, false); // disables strict mode
</code></pre>

<h3>shardkey (new in v2.5.3)</h3>
...
<p>In this example, Mongoose will include the required <code>tag</code> and <code>name</code> in all <code>doc.save()</code> <code>where</code> clauses for us.</p>
