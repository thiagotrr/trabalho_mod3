...
<p><code>Schema</code>s are pluggable, that is, they allow for applying pre-packaged<br />capabilities to extend their functionality.</p>

<p>Suppose that we have several collections in our database and want<br />to add last-modified functionality to each one. With plugins this<br />is easy. Just create a plugin once and apply it to each <code>Schema</code>:</p>
...
<p>In the example above we added last-modified functionality to both the Game and Player schemas. We also took advantage of options passing supported by the <code>plugin()</code> method to dynamically define an index on the Player.</p>
        </div>
